...
def generate_opencode_plan(key: str, profile: dict[str, Any], job_id: str) -> dict[str, Any]:
    ...
    day_shape = """Hver dag skal have: day (tal), name, focus, meals med breakfast/lunch/dinner/snack. Hvert måltid har title, ingredients med konkrete mængder til 1 person, portion, prepMinutes og method med 4-6 handlingsklare trin i rigtig rækkefølge. Trinene skal angive relevant temperatur, aktiv tid, tilberedningstid og et synligt eller målbart tegn på, at maden er færdig; skriv aldrig bare 'tilbered råvarerne'. movement har type, title, minutes, intensity, instructions (liste) og alternative. Desuden habit og encouragement."""
    prompts = {
